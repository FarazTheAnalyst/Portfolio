#----------------------CREATING CSV FILE FOR POWER BI----------------------
...
#load original .csv files, a years worth of data frame aug 2020 to july 2021
...
#calculate the ride length by subtracting the ending_at time from started_at time and converted it to minutes
...
cyclistic_date$day_of_week <- format(as.Date(cyclistic_date$date), format = "%A") #create column for day of week
cyclistic_date$month <- format(as.Date(cyclistic_date$date), format = "%m") #create column for month
cyclistic_date$day <- format(as.Date(cyclistic_date$date), "%d") #create column for month
...
cyclistic_date <- cyclistic_date[!(cyclistic_date$ride_length <=0),] #remove where ride length is 0 or negative
...
#created new data frame to use in Power bi
cyclistic_powerbi <- cyclistic_date

#clean the data
cyclistic_powerbi <- cyclistic_powerbi %>% #remove the columns not needed: start_station_name, end_station_name, time, started_at, ended_at
  select(-c(start_station_name, end_station_name, time, started_at, ended_at))

#download the new data as a .csv file
fwrite(cyclistic_powerbi, "cyclistic_data.csv")
